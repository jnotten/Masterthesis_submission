...
'''
Experiment over the number of classifiers.
'''
...
STEP = 1
MAXNUMBER = 300
NUMBERS = [i for i in range(STEP,MAXNUMBER,STEP)]
MTRY =  'log'
...
# The numbers wie use for the experiment with extra list if we want to stop with our own Walltime.
stop_numbers = NUMBERS
# Iterate over all the datasets we have passed over the command line.
for name in featuresToDo:
    # Output in the command line for overview.
    print('________________________________________________________')

    str1 = f'Baseclassifier:{BASECLASSIFIER} and parameter {PARAMETER} \n + oobcompute={OOBCOMPUTE},cart={CART},c4_5={C4_5},chaid={CHAID},Feature|multiply={FEATURES_MULTIPLY},rotate={FEATURES_ROTATE},linearComb={FEATURES_LINEARCOMBINATION} , weight_ensemble={WEIGHT_ENSEMBLE},weight_features={WEIGHT_FEATURE}'
    print(str1)
    str2 =f'dataset: {name} & number of trees: Steps: {STEP} total:{MAXNUMBER}'
    print(str2)
    print(
    f'normal vs  oobcompute={OOBCOMPUTE},cart={CART},c4_5={C4_5},chaid={CHAID},Feature|multiply={FEATURES_MULTIPLY},rotate={FEATURES_ROTATE},linearComb={FEATURES_LINEARCOMBINATION}, weight_ensemble={WEIGHT_ENSEMBLE},weight_features={WEIGHT_FEATURE}')
    ...
    # Initialize the two own Framework models with the parameter we get and one without all modifications.
    ...
    # Initialize help variables and iterate through the numbers of classifiers.
    maintime = time.time()
    maxMcNemar = -1
    for number in NUMBERS:

        # Check if we under the walltime.
        tmp_time = time.time() - maintime
        if tmp_time >= WALLTIME:
            stop_numbers = [i for i in range(STEP,number,STEP)]
            break

        # Initialize the models of sklearn and train all four models with the training data.
        rf.fit(train, y_train, perCent=PERCENT, number=STEP, cat_ix=cat_ix_var)
        rf_modify.fit(train, y_train, perCent=PERCENT, number=STEP, cat_ix=cat_ix_var)
    ...
    measur['numberOfClassifier'] = NUMBERS
    save_df = pd.DataFrame.from_dict(measur)
    save_df = save_df.set_index('numberOfClassifier')
    save_df.to_csv(f'{user_paths[0]}/data_numberOfClassifier/classifiers_{name}_{MTRY}_{s_str}.csv')
