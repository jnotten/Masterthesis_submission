...
'''
Plotting the experiments number of features.
'''
...
for mtry in VARIANTS:
    for s_str in MODIFICATIONS:
        ...
        try:
            ...
            measur = pd.read_csv(f'{user_paths[0]}/data_numberOfFeatures/features_{name}_{mtry}_{s_str}.csv')
            ...
            fig,ax = plt.subplots(nrows=7,ncols=1,figsize=(15,30),sharex='row')
            #plt.suptitle(str1 + '\n' + str2)
            n = len(DIRECTMEASURES)
            for i in range(n):
                ax[i].plot(numbers, measur[DIRECTMEASURES[i]],linewidth=4)
                ...
                if DIRECTMEASURES[i] in ['test accuracy', 'training accuracy', 'micro f1', 'macro f1']:  # , 'oobscore']:
                    ax[i].plot(numbers, measur[DIRECTMEASURES[i] + LABEL_SKLEARN],linewidth=4)
                    ax[i].plot(numbers, measur[DIRECTMEASURES[i] + LABEL_REGRESSION],linewidth=4)
                    ...
                    if i == 4:
                        ...
                        ax[i].legend(['rf_normal', 'rf_modify'], loc='best')
            ...
            plt.savefig(f'{user_paths[0]}/figures_numberOfFeatures/features_{name}_{s_str}_{mtry}.png', dpi=100) #_{now}_{s_str}
            #plt.show()
            plt.close(fig)
        except:
